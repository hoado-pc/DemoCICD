...
security import ./scripts/certs/qc.p12 -k $KEY_CHAIN -P Highjump1@  -T /usr/bin/codesign
...
uuid=`grep UUID -A1 -a ./scripts/profiles/cicd_qc.mobileprovision | grep -io "[-A-F0-9]\{36\}"`
cp ./scripts/profiles/cicd_qc.mobileprovision ~/Library/MobileDevice/Provisioning\ Profiles/$uuid.mobileprovision
